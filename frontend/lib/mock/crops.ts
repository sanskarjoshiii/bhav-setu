...
export interface Crop {
  ...
  /** ₹ per quintal, the belt-wide reference the per-mandi price is derived from */
  basePrice: number;
  /** 1 = very perishable … 5 = storable */
  perishability: number;
  /** spoilage rate constant used by the economics engine */
  kC: number;
...
export const VEGETABLES = CROPS.filter((c) => c.category === "vegetable");
export const FRUITS = CROPS.filter((c) => c.category === "fruit");

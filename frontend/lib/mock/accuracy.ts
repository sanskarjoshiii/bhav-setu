import type { AccuracySummary, BacktestPoint } from "../types";
...
export const ACCURACY: AccuracySummary = {
  mape: [
...
export const CALIBRATION: CalibrationBin[] = [
...
export function residuals(): { predicted: number; actual: number }[] {
  const rand = mulberry32(20260811);
  const out: { predicted: number; actual: number }[] = [];
  for (let i = 0; i < 120; i++) {

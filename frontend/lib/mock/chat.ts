...
/**
 * The scripted bot. Phase 9 replaces this with the real engine over /api/chat —
 * the component only ever calls `openingMessages` and `nextReply`, so that swap
 * is two functions.
 *
 * THE DEFAULT FLOW (each step waits for the farmer, and every step offers
 * quick-reply buttons so nothing has to be typed):
 *
 *   1. GREETING          bot introduces itself, asks crop + quantity
 *   2. AWAITING_CROP     farmer answers e.g. "onion 80 quintal"
 *   3. AWAITING_GRADE    bot asks grade  -> A / B / C
 *   4. AWAITING_STORAGE  bot asks storage -> shed / ambient / cold store
 *   5. ADVICE_GIVEN      bot returns the split plan, the reason, confidence,
 *                        and the rupee gain versus selling everything today
 *   6. AWAITING_SALE     farmer taps "I sold today"; bot asks the real price
 *   7. RECORDED          bot thanks them and says it is on the History page
 *
 * Anything unrecognised falls through to a help message rather than a dead end.
 */
...
const SCRIPT: Record<Step, Payload> = {
  greeting: {
    en: [
...
const ORDER: Step[] = ["greeting", "crop", "grade", "storage", "advice", "sale", "recorded"];

const MATCHERS: Partial<Record<Step, RegExp>> = {
...
function toMessages(payload: Payload, lang: Language, tag: string): ChatMessage[] {
...
export function openingMessages(lang: Language): ChatMessage[] {
  cursor = 1;
...
export function nextReply(input: string, lang: Language): ChatMessage[] {

...
export const DISTRICTS: string[] = Array.from(new Set(MANDIS.map((m) => m.district)));

/** The five the advisor and comparison default to — one district, walkable set. */
export const HOME_MANDIS = MANDIS.filter((m) => m.district === "Nashik");

export const REFERENCE_VILLAGE = {
  name: "Vinchur",
  nameMr: "विंचूर",
  district: "Nashik",
  lat: 20.11,
...
export function mandiByName(name: string): Mandi {
  return MANDIS.find((m) => m.name === name) ?? MANDIS[0];
}

export function mandisInDistrict(district: string): Mandi[] {
  return district === "All" ? MANDIS : MANDIS.filter((m) => m.district === district);
}

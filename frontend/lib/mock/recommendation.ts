import type { Grade, Recommendation, RiskProfile, Storage, Tranche } from "../types";
import { HOME_MANDIS } from "./mandis";
import { cropById } from "./crops";
import { compareMandis, netInHand, spoilageFraction } from "./economics";
import { seriesFor } from "./prices";
import { longDate } from "../seed";
 ...
 */
export function recommend(lot: LotInput): Recommendation {
  ...
  if (holdDays.length < 3) constraints.push("max_hold_days");

  holdDays = holdDays.filter((d) => spoilageFraction(d, lot.storage, lot.cropId) <= 0.15);
  if (holdDays.length === 0) {
    holdDays = [Math.max(1, Math.min(3, crop.maxHoldDays))];
    constraints.push("spoilage_cliff");
  }

  const ranked = compareMandis(lot.qtyQtl, 0, lot.grade, lot.storage, lot.cropId);
  const bestNow = ranked[0];
  const nearest = [...HOME_MANDIS].sort((a, b) => a.distanceKm - b.distanceKm)[0];

  const baseline = netInHand({
    pricePerQtl: seriesFor(nearest.name, lot.cropId).filter((p) => !p.isForecast).slice(-1)[0]
      .modal!,
    qtyQtl: lot.qtyQtl,
    daysHeld: 0,
  ...
  const pool = minViable ? HOME_MANDIS.filter((m) => m.distanceKm <= 40) : HOME_MANDIS;
  if (minViable) constraints.push("min_viable_load");
  ...
  for (const fraction of sellFractions) {
    for (const days of holdDays) {
      for (const later of pool) {
        considered++;
        const forecast = seriesFor(later.name, lot.cropId).filter((p) => p.isForecast);
        ...
        const nowNet =
          nowQty > 0
            ? netInHand({ pricePerQtl: bestNow.grossPerQtl, qtyQtl: nowQty, daysHeld: 0, distanceKm: bestNow.distanceKm, grade: lot.grade, storage: lot.storage, cropId: lot.cropId }).netTotal
            : 0;
        const laterNet =
          laterQty > 0
            ? netInHand({ pricePerQtl: at.p50, qtyQtl: laterQty, daysHeld: days, distanceKm: later.distanceKm, grade: lot.grade, storage: lot.storage, cropId: lot.cropId }).netTotal
            : 0;
        const laterLow =
          laterQty > 0
            ? netInHand({ pricePerQtl: at.p10!, qtyQtl: laterQty, daysHeld: days, distanceKm: later.distanceKm, grade: lot.grade, storage: lot.storage, cropId: lot.cropId }).netTotal
            : 0;
  ...
  const laterMandiObj = HOME_MANDIS.find((m) => m.name === best.laterMandi)!;
  const laterForecast = seriesFor(best.laterMandi, lot.cropId).filter((p) => p.isForecast)[
    best.days - 1
  ];
  ...
  if (nowQty > 0) {
    const n = netInHand({ pricePerQtl: bestNow.grossPerQtl, qtyQtl: nowQty, daysHeld: 0, distanceKm: bestNow.distanceKm, grade: lot.grade, storage: lot.storage, cropId: lot.cropId });
  ...
  if (laterQty > 0 && laterForecast?.p50) {
    const mk = (price: number) =>
      netInHand({ pricePerQtl: price, qtyQtl: laterQty, daysHeld: best.days, distanceKm: laterMandiObj.distanceKm, grade: lot.grade, storage: lot.storage, cropId: lot.cropId }).netPerQtl;
    tranches.push({
      qtl: laterQty,
      when: longDate(laterForecast.date),
      dayOffset: best.days,
...
export const DEFAULT_LOT: LotInput = {
  cropId: "onion",
  qtyQtl: 80,
  grade: "B",
  storage: "shed",
